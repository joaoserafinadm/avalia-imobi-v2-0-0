...
export default authenticated(async (req, res) => {
    if (req.method === 'GET') {
        ...
        let { active, errorStatus, dateLimit, pixPaymentData } = companyExist

        // Verificar tanto dateLimit quanto subscription_limit_date
        const dateLimitValid = dateLimitValidate(dateLimit)
        const subscriptionValid = subscriptionLimitValidate(pixPaymentData?.subscription_limit_date)

        console.log("dateLimitValid", dateLimitValid)
        console.log("subscriptionValid", subscriptionValid)

        // Se qualquer uma das validações falhar, desativar a empresa
        if (active && !errorStatus && (!dateLimitValid || !subscriptionValid)) {
...
// Nova função para validar a data limite da assinatura PIX
const subscriptionLimitValidate = (subscriptionLimitDate) => {
    // Se não existe data de assinatura, considerar válido (pode ser usuário sem PIX)
    if (!subscriptionLimitDate) return true

    const dateObj = new Date(subscriptionLimitDate)
    if (isNaN(dateObj)) {
        console.error("Invalid subscription date:", subscriptionLimitDate)
        return false // Se a data é inválida, considerar como vencida
    }

    const currentDate = new Date()
    currentDate.setHours(0, 0, 0, 0) // Remove time part

    console.log("subscriptionLimitDate", dateObj, currentDate)
