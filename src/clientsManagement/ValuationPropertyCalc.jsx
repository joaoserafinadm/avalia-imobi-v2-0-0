export default function ValuationPropertyCalc({ client }) {
    ...
    return (
        <div>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', color: 'var(--theme-text-faint)', marginBottom: '1rem' }}>
...
function CalcCard({ label, value, prefix, suffix, mono, highlight }) {
    return (
        <div style={{
            background: highlight ? 'rgba(245,135,79,0.07)' : 'var(--theme-section-bg)',
            border: highlight ? '1px solid rgba(245,135,79,0.25)' : '1px solid var(--theme-border-subtle)',
            borderRadius: '12px', padding: '16px', textAlign: 'center',
        }}>
            <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.68rem', color: 'var(--theme-text-faint)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>
                {label}
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: '4px', flexWrap: 'wrap' }}>
                ...
                <span style={{
                    ...
                    color: highlight ? '#f5874f' : 'var(--theme-text-primary)',
                }}>
                    {value ?? '—'}
                </span>
                {suffix && <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.8rem', color: 'var(--theme-text-faint)' }}>{suffix}</span>}
            </div>
        </div>
    )
}
...
function PricingCard({ label, value, badge, badgeColor, badgeTextColor, selected }) {
    return (
        <div style={{
            background: selected ? 'rgba(245,135,79,0.06)' : 'var(--theme-section-bg)',
            border: selected ? '1.5px solid rgba(245,135,79,0.4)' : '1px solid var(--theme-border-subtle)',
            borderRadius: '12px', padding: '20px', textAlign: 'center',
            boxShadow: selected ? '0 0 0 3px rgba(245,135,79,0.08)' : 'none',
            transition: 'border-color 0.2s ease, box-shadow 0.2s ease',
        }}>
            <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', color: 'var(--theme-text-faint)', marginBottom: '10px' }}>
                {label}
            </div>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: '4px', marginBottom: '10px' }}>
                ...
                <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: '1.25rem', fontWeight: 500, color: 'var(--theme-text-primary)' }}>
